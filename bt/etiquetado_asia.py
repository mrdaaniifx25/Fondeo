...
DESDE, HASTA = "2020-01-01", "2026-01-01"     # 2026 se reserva
N_POR_ANO = 10
SEMILLA = 20260827
...
for dia, g in v.groupby("dia"):
    ...
    for i in range(i0, i1 + 1):
        ...
        for k in (1, 2):
            ...
            break
        break            # solo el primer barrido del dia, haya gatillo o no
...
# muestra estratificada por ano
elegidos = []
for ano, g in s.groupby("ano"):
    k = min(N_POR_ANO, len(g))
    elegidos.append(g.iloc[rng.choice(len(g), size=k, replace=False)])
sel = pd.concat(elegidos).sort_values("ts").reset_index(drop=True)
...
print(f"candidatos {len(s):,}  ·  elegidos {len(sel)}  ·  "
...
json.dump(fichas, open("data/etiquetado_asia_setups.json", "w"))
sel.merge(pd.DataFrame(mec), on="id").to_csv("data/etiquetado_asia_verdad.csv", index=False)
pd.concat(caminos).to_parquet("data/etiquetado_asia_camino.parquet", index=False)
